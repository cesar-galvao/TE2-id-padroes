...
cowplot::plot_grid(p1, p2, p3,p4, nrow = 1, labels = c("class_1", "class_2", "class_3","class_4"))

```

## Ajuste do modelo

A seguir, ajustamos o modelo MclustDA para as classificações `class_1`, `class_2` , `class_3` e `class_4` da base de treino:
...
knitr::kable(mod3_conf <- table(dados$class_3, predict(mod3)$classification))
```
...
pond <- mod3$prop * c(25, 1) # proporções das classes * custo
mod3_pred_custos <- predict(mod3, prop = pond / sum(pond))
```
...
knitr::kable(mod3_custos_conf <- table(dados$class_3, mod3_pred_custos$classification))
```
\ 
...
pred <- dados_teste %$% predict(mod5, newdata = tibble(eruptions,waiting))

classError(pred$classification, dados_teste$class_4)$error
...
prioprob <- dados_teste%$%classPriorProbs(mod5, cbind(eruptions, waiting))
prioprob

pred1 <- dados_teste%$% predict(mod1,newdata=cbind(eruptions,waiting),prop=prioprob)

classError(pred1$classification, dados_teste$class_4)$error
